...
#define LCDC_GET_BGWINDOW_TILEDATA_AREA() ((ppu.reg_lcdc & (1 << 4)) >> 4) // select the addressing mode
#define LCDC_GET_BG_TILEMAP_AREA() ((ppu.reg_lcdc & (1 << 3)) >> 3) // select which area in the memory to fetch bg map from
...
struct ppu {
    ...
    uint8_t scrollX; // specify the top left coordinates of the visible parts of the bg map
    uint8_t scrollY;

    // scan line compare
    uint8_t LYC; // should be compared to LY and LY and the flag in stat should be set if LYC == LY
                // LY is scanline
...
struct pixelFetcher
{
    ...
    uint8_t WLY; // internal scanline counter for the window, incremented everytime a scanline has window elements
                // and reset every V-BLANK
